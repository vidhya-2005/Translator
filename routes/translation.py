...
def file_translation():
    ...
    try:
        ...
        token = uuid.uuid4().hex
        input_path = os.path.join(_tmp_dir(), f"{token}{extension or os.path.splitext(file.filename)[1].lower()}")
        file.save(input_path)
        if audio_video: return jsonify(process_media(input_path, file.mimetype or "application/octet-stream", target, source))
        if extension == ".docx":
...
def translate_visual():
    ...
    try:
        ...
        tmp = _tmp_dir()
        input_path = os.path.join(tmp, f"{token}{ext}")
        output_ext = ".pdf" if ext == ".pdf" else (".jpg" if ext in (".jpg", ".jpeg") else ".png")
        ...
        if ext == ".pdf":
            ...
            name, mime = f"{os.path.splitext(file.filename)[0]}_translated{output_ext}", mime_type
        return jsonify({**result, **_media_download_response(output_path, name, mime)})
    except Exception as exc: return _error_response(exc)
    finally:
...
def word_translation():
    ...
    try:
        ...
        if not original_text: return jsonify(error="No readable text found in the Word document."), 400
        ...
        return jsonify({"detected_language_name": "Auto-detected" if source == "auto" else source, "detected_language_code": source, "transcription": original_text, "translation": translated_text, **media})
    except Exception as exc: return _error_response(exc)
    finally:
        for path in (input_path, output_path):
            if path and os.path.exists(path):
