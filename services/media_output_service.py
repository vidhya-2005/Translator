...
def generate_tts(text, api_key, output_path, language_name="English"):
    ...
    payload = {
        ...
        "generation_config": {"speech_config": [{"voice": "Kore", "language": _language_code(language_name)}]},
    }
    last_error = None
    for attempt in range(3):
        try:
            ...
            last_error = f"Gemini TTS error ({response.status_code}): {message}"
            if response.status_code not in (429, 500, 502, 503, 504):
                break
        except requests.RequestException as exc:
            last_error = f"Gemini TTS request failed: {exc}"
        if attempt < 2:
            time.sleep(2 ** attempt)
    raise ValueError(last_error or "Gemini TTS failed.")
...
def _language_code(language_name):
    codes = {
        ...
        "Gujarati": "gu-IN", "Punjabi": "pa-IN", "Urdu": "ur-IN", "Spanish": "es-ES",
...
def video_with_translated_audio(video_path, translated_audio_path, output_path):
    _run(["-y", "-i", video_path, "-i", translated_audio_path, "-map", "0:v:0", "-map", "1:a:0", "-c:v", "copy", "-c:a", "aac", "-af", "apad", "-shortest", output_path])
